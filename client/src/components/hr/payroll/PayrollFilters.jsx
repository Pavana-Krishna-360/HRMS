...
const PayrollFilters = ({ activeFilter, setActiveFilter, searchQuery, setSearchQuery }) => {
    ...
    <div className="space-y-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-100 pb-4">
        <div className="flex items-center gap-4 w-full sm:w-auto justify-center">
          ...
          <span className="font-bold text-slate-800 text-lg">August 2026</span>
          <button className="p-2 border border-slate-200 rounded-md hover:bg-slate-50 text-slate-600 transition-colors">
            <ChevronRight className="w-4 h-4" />
          </button>
        </div>
        <Button variant="outline" className="w-full sm:w-auto text-sm py-2 px-4 h-10">
          This Month
        </Button>
      </div>
